...
by_group = ddply(d_valid, .(Group_ID, Political, Condition), summarize
...
melt(by_group, id.vars=c("Group_ID","Political","Condition","N")) %>%
...
paired=ddply(soc, .(Group_ID, Condition), summarize
  # Calculate difference (polarization) at round 1
  , diff_1 = (correct1[Political=="Conservative"]-correct1[Political=="Liberal"])
  
  # Calculate difference (polarizaiton) at round 2
  , diff_3 = (correct3[Political=="Conservative"]-correct3[Political=="Liberal"])
